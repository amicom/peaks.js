...
define(['./../utils','d3-array','d3-scale','d3-shape'], function(Utils,ArrayUtil,Scale,Shape) {
  'use strict';
  ...
  WaveformShape.prototype.draw = function(context) {
    var v = this._view;
    var frameOffset = v.getFrameOffset();
    ...
    var offsetX = 100;
    var waveform = v._originalWaveformData;
    console.log(waveform);
    var channel = waveform.channel(0);
    var waveformLayer = v._waveformLayer;

    var min = channel.min_array();
    var max = channel.max_array();
    // x.domain([0, waveform.length]).range([0, v.getWidth()]);
    x.domain([frameOffset, waveform.length]).rangeRound([0, v.getWidth()]);
    x.clamp(true);
    y.domain([ArrayUtil.min(min), ArrayUtil.max(max)]).rangeRound([offsetX, -offsetX]);

    // var w = v.getWidth();
    // setInterval(function(){
    //   frameOffset+=10;
    //   console.log("hello");
    //   x.domain([frameOffset, 1000+frameOffset]).rangeRound([0, w]);
    // }, 500);
  ...
  waveformLayer.append('path').datum(max)
  .attr("stroke", this.color)
  .attr("fill", this.color)
  .attr('transform', () => `translate(0, ${v.getHeight()/2})`)
  .attr('d', area);
};
